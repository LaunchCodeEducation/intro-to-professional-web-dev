...
The ``for`` loop is the first JavaScript tool for iteration that we will explore. A **for loop** allows us to carry out **definite iteration**. Definite iteration is the process of repeating a specific task with a specific data set. In other words, when a ``for`` loop begins it can usually be determined exactly how many times it will execute: once for each item in the data set.
...
This program prints the integers 0 through 50. In the language of definite iteration, we can say that the operation that this loop carries out printing a value to the console, and its data set is the collection of integers 0...50.
...
- The boolean expression ``i < 51`` is the **loop condition**. This condition is evaluated before each loop iteration, or repetition. If the condition is ``true`` then the loop executes again. If the condition is ``false`` then the loop ceases execution and the program continues to execute the code below the loop. In the example above, an equivalent condition would be ``i <= 50``. It is common to use ``<`` when writing loop conditions, so that the last value used in the loop is *one less than* then right-hand side of the ``<`` symbol.
