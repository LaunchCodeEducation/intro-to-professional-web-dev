...
While not used often, ``break`` can be quite helpful in some situations. Consider a situation where we are searching for a particular element in an array. (While we have seen that JavaScript has array methods that can carry out array searches, many programming languages do not.) 

We can use a ``while`` loop to say, essentially, *while we have not reached the end of the array, continue iterating*. We can then include a ``break`` within a conditional check to say, *when we have found the element we are searching for, exit the loop*.
...
Notice that we use a ``while`` loop rather than a ``for`` loop to iterate over the array. This is because our loop variable, ``i``, is used outside the loop. When we use a ``for`` loop in the way we have been, the loop variable exists only within the loop.
